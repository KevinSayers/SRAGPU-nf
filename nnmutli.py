...
def runModel(encodedFile):
	...
	for j in temp[0:342]:
		...
		if j[1] == "triple-negative breast cancer (TNBC)":
			counttrain += 1
			traininglabels.append(np.asarray([1,0,0,0]))
		if j[1] == "human epidermal growth factor receptor 2 positive (HER2+)":
			traininglabels.append(np.asarray([0,1,0,0]))
		if j[1] == "estrogen receptor positive (ER+)":
			traininglabels.append(np.asarray([0,0,1,0]))
		if j[1] == "double positive (ER+ and HER2+)":
			traininglabels.append(np.asarray([0,0,0,1]))
	...
	for j in temp[343:]:
		...
		if j[1] == "triple-negative breast cancer (TNBC)":
			counttest += 1
			testlabels.append(np.asarray([1,0,0,0]))
		if j[1] == "human epidermal growth factor receptor 2 positive (HER2+)":
			testlabels.append(np.asarray([0,1,0,0]))
		if j[1] == "estrogen receptor positive (ER+)":
			testlabels.append(np.asarray([0,0,1,0]))
		if j[1] == "double positive (ER+ and HER2+)":
			testlabels.append(np.asarray([0,0,0,1]))
...
# W = tf.Variable(tf.zeros([540728, 2]))
	W = tf.Variable(tf.truncated_normal([inputs, classes], stddev=0.1))
